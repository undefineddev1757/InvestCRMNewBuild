...
if [ "$USE_BUILTIN_NGINX" = true ]; then
    ...
    echo ""
    echo "🔧 НАСТРОЙТЕ ДОМЕН АВТОМАТИЧЕСКИ:"
    echo "   sudo bash ${PROJECT_ROOT}/setup_domain.sh YOUR_DOMAIN"
    echo ""
    echo "   Например:"
    echo "   sudo bash ${PROJECT_ROOT}/setup_domain.sh invest-platform.top"
    echo ""
    echo "   Скрипт автоматически:"
    echo "   • Настроит nginx"
    echo "   • Получит SSL сертификат"
    echo "   • Обновит .env файл"
fi
